...
# Setup de ambiente para garantir resolução de módulos em produção (Streamlit Cloud)
...
def set_all_state(label, options, value):
    """Callback de sincronia: Garante que os checkboxes mudem de estado antes do rerun."""
    for opt in options:
        st.session_state[f"chk_{label}_{opt}"] = value

def run_dashboard():
    """Interface Black Crow Intel - Nixtla Forecast & Diagnóstico SPC."""
    st.set_page_config(page_title=APP_TITLE, layout="wide")

    # CSS Customizado: Estética Dark Mode & Métricas de Alta Densidade
    st.markdown(f"""
        <style>
    ...
    if df_raw.is_empty(): st.error("❌ Erro: Base indisponível."); st.stop()

    # --- SIDEBAR: SEGMENTAÇÃO COMPACTA ---
    with st.sidebar:
        ...
        st.caption("Black Crow Intelligence | v2.0.0 Nixtla Ready")
    ...
    if not df_filt.is_empty():
        # Cálculos de Inteligência & Predição Nixtla
        ...
        # Forecast por Cliente (Nixtla Logic)
        df_forecast = PredictionService.get_client_predictions(df_filt)
        proj_total, trend = PredictionService.get_market_trend(df_filt)
        ...
        k5.metric("Forecast Total", f"{proj_total} un", delta=trend)

        st.divider()

        # --- BLOCO 2: VISUALIZAÇÃO ---
        st.subheader("📊 Diagnóstico de Trajetória & Forecast")
        ...
        with c1:
            ...
            for val, color, txt in [(ucl, "#238636", "UCL"), (lcl, "#da3633", "LCL")]:
                fig_spc.add_hline(y=val, line_dash="dash", line_color=color, annotation_text=txt)
            st.plotly_chart(fig_spc, use_container_width=True)

        with c2:
            st.subheader("🔮 Predição de Compras Futuras (Nixtla)")
            if not df_forecast.is_empty():
                st.dataframe(df_forecast, use_container_width=True, hide_index=True)
            else:
                st.info("Aguardando volume de dados para processar forecast nominal.")

        st.divider()

        # --- BLOCO 3: LOGIC ENGINE ---
        ...
        - Previsibilidade: Nota de Confiança em {max(0, 100-(vol_cv*100)):.1f}% (Base estatística Nixtla/Sigma-2).
        - Insight de Compra: {len(df_forecast)} clientes com propensão de compra detectada para a próxima janela.
        - Estabilidade: {'Atenção: Alta flutuação detectada.' if vol_cv > 0.4 else 'Fluxo operacional em regime de normalidade estatística.'}
        ```
        """, unsafe_allow_html=True)

...
# Setup de ambiente: Resolve caminhos de módulos em ambientes de deploy
...
def run_dashboard():
    ...
    # Injeção de CSS para identidade Dark e métricas compactas
    st.markdown(f"""
        <style>
    ...
    if df_raw.is_empty():
        st.error("❌ Base de dados fct_sales não encontrada."); st.stop()
    ...
    with st.sidebar:
        ...
        st.caption("v2.2.8 | Platinum Nixtla Edition")
    ...
    if not df_filt.is_empty():
        ...
        if not dist_data.is_empty():
            ...
            k2.metric("Líder de Mix", f"{dist_data['marca'][0][:12]}...", delta=f"{share:.1%} Share")
        ...
        if not df_forecast.is_empty():
            ...
            st.info("💡 Volume insuficiente para gerar predições nominais no contexto selecionado.")

...
# --- CONSTANTES DE DESIGN SISTÊMICO (CONTRASTE ABSOLUTO) ---
LEGEND_COLOR = "#0F172A"
AXIS_COLOR = "#000000"
...
def apply_enterprise_styles():
    ...
    st.markdown(f"""
        ...
        /* Rótulo da Métrica (O título do card) */
...
def run_dashboard():
    ...
    if not df.is_empty():
        ...
        st.markdown(f"<p style='color:#475569; font-weight:900; text-transform:uppercase; letter-spacing:2px; font-size:1.1rem;'>Intelligence Engine • v6.0.5</p>", unsafe_allow_html=True)
        ...
        if not dist.is_empty():
            try:
                lider_text = dist.tail(1)['marca'][0]
            except (KeyError, IndexError):
                lider_text = dist.tail(1).to_series(0)[0]
        ...
        # --- ROW 2: SPC HERO ---
        st.markdown("<h3>📈 1. Estabilidade e Tendência</h3>", unsafe_allow_html=True)
        ...
        with c1:
            st.markdown("<h3>🏆 2. Dominância de Mercado</h3>", unsafe_allow_html=True)
            if not dist.is_empty():
        ...
        st.divider()
        st.markdown("<h3>🔮 4. Antecipação de Vendas (Próximo Ciclo)</h3>", unsafe_allow_html=True)
        ...
        <div class="insights-card">
            <h4 style="margin-top:0; color:{LEGEND_COLOR}; font-weight:900; text-transform:uppercase; border-bottom:2px solid #F1F5F9; padding-bottom:15px;">Sumário de Inteligência</h4>
            <div style="display: grid; grid-template-columns: repeat(4, 1fr); gap: 30px; margin-top:25px;">
                <div><span style="font-size:0.8rem;">PERFIL CARTEIRA</span><br><div style="color:{LEGEND_COLOR}; font-weight:900; font-size:1.8rem;">{ins.get('perfil').upper()}</div></div>
                <div><span style="font-size:0.8rem;">CONCENTRAÇÃO</span><br><div style="color:{LEGEND_COLOR}; font-weight:900; font-size:1.8rem;">{ins.get('hhi',0):.3f}</div></div>
                <div><span style="font-size:0.8rem;">ESTADO</span><br><div style="color:{LEGEND_COLOR}; font-weight:900; font-size:1.8rem;">{ins.get('estabilidade').upper()}</div></div>
                <div><span style="font-size:0.8rem;">VARIABILIDADE</span><br><div style="color:{LEGEND_COLOR}; font-weight:900; font-size:1.8rem;">{(ins.get('cv',0)*100):.1f}%</div></div>
            </div>
        </div>
        """, unsafe_allow_html=True)
